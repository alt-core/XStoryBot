...
class LineImageTextPlugin_Builder(object):
    def __init__(self, params):
        self.more_message = params['more_message']
        ...
        for key in frames.keys():
            self.frames[key] = utility.merge_params({
                ...
                'margin_x': 0,
                'margin_y': 0,
            }, utility.extract_params(frames[key], ['size_x', 'size_y', 'margin_x', 'margin_y']))
    ...
    def build_from_command(self, builder, msg, options):
        text = options[0]
        ...
        while text:
            ...
            if stat is None:
                png_data, rest = renderer.render_text_to_png(text, frame_opt['size_x'], frame_opt['size_y'], frame_opt['margin_x'], frame_opt['margin_y'], **frame_opt['text_rendering_options'])
            ...
            builder.add_command(default_commands.IMAGEMAP_CMDS[0], [unicode(image_url), unicode(size[0]), unicode(size[1])], [])
            text = rest
            if text:
                # 表示残しがある
                filepath, size = builder.build_image_for_imagemap_command(self.more_image_url)
                builder.add_command(default_commands.IMAGEMAP_CMDS[0], [unicode(filepath), unicode(size[0]), unicode(size[1])], [[u'0,0,{},{}'.format(size[0],size[1]), self.more_message]])
                next_label = u'##IMGTEXT__{}__{}'.format(builder.scene.get_relative_position_desc(builder.node), counter)
                builder.add_command(more.SET_NEXT_LABEL_CMD, [next_label,], None)
