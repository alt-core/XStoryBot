...
class PlayerNextLabelDB(object):
    ...
    @ndb.transactional
    def set_next_label(self, label, status):
        entry_next_label = PlayerNextLabel.get_by_id(status.id)
        overwrite = None
        if not entry_next_label:
            entry_next_label = PlayerNextLabel(id=status.id, next_label=label)
        else:
            if entry_next_label.next_label:
                overwrite = entry_next_label.next_label
    ...
    def get_next_label(self, status):
        ...
        if entry_next_label is None:
            return None
        return entry_next_label.next_label
    ...
    def compare_and_clear_next_label(self, status, next_label):
        ...
        if entry_next_label is None:
            return None
        ret = entry_next_label.next_label
        if ret == next_label:
...
class LineMorePlugin_Runtime(object):
    ...
    def run_command(self, context, msg, options):
        if msg == SET_NEXT_LABEL_CMD:
            overwrite = PlayerNextLabelDB().set_next_label(options[0], context.status)
            if overwrite:
                logging.warning(u'exec set next label command: {} overwrites {}'.format(options[0], overwrite))
            else:
    ...
    def modify_incoming_action(self, context, action):
        ...
        for retry in range(10):
            next_label = db.get_next_label(context.status)
