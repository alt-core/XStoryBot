...
class GroupMessageTaskDB:
    ...
    def _store_member_list(task_id, members):
        ...
        try:
            ...
            logging.info(f"Stored member list for task {task_id} to private object storage.")
            return reference
        except ObjectStoreError as e:
            logging.error(f"Failed to store member list for task {task_id} to private object storage: {type(e).__name__} - {str(e)}")
            raise
    ...
    def get_members_from_storage(task_id):
        try:
            ...
            logging.error(f"Member list file not found in private object storage for task {task_id}: {str(e)}")
            raise # ファイルがない場合は致命的なので例外を発生させる
        except ObjectStoreError as e:
            logging.error(f"Failed to get member list for task {task_id} from private object storage: {type(e).__name__} - {str(e)}")
            raise
        ...
        def build_update(data):
            ...
            if failed_member_id is not None:
                ...
                try:
                    ...
                    # ObjectStoreへの追記失敗はログに残し、StateStoreの更新は続行する。
                    logging.error(f"Failed to append failed member {failed_member_id} to private object storage for task {task_id}: {str(storage_error)}")

                # StateStoreにはUI表示用の最新N件のみを保持する。
                failed_ids = list(data.get('failed_member_ids', []))

                if failed_member_id not in failed_ids:
                    if len(failed_ids) >= GroupMessageTaskDB.MAX_FAILED_IDS_IN_DB:
                        # 古いものから削除 (リストの末尾に追加される想定なら pop(0))
    ...
    def _store_failed_member_list(task_id, failed_members):
        try:
            ...
            logging.info(f"Stored failed member list for task {task_id} to private object storage.")
            return reference
        except ObjectStoreError as e:
            logging.error(f"Failed to store failed member list for task {task_id} to private object storage: {type(e).__name__} - {str(e)}")
            raise
    ...
    def process_members_in_parallel(task_id, process_function, max_workers=None,
        ...
        # 処理完了後、成功メンバーリストをObjectStoreに保存
    ...
    def _store_successful_members(task_id, successful_members):
        try:
            ...
            logging.info(f"Stored successful members list for task {task_id} to private object storage.")
        except ObjectStoreError as e:
            logging.error(f"Failed to store successful members list for task {task_id} to private object storage: {type(e).__name__} - {str(e)}")
    ...
    def _store_error_logs(task_id, error_logs):
        try:
            ...
            logging.info(f"Stored error logs for task {task_id} to private object storage.")
        except ObjectStoreError as e:
            logging.error(f"Failed to store error logs for task {task_id} to private object storage: {type(e).__name__} - {str(e)}")
            # このエラーは処理結果の記録に関するものなので、ログ出力に留める
    ...
    def get_successful_members(task_id):
        try:
            ...
            logging.debug(f"successful_members.json not found in private object storage for task {task_id}. Returning empty list.")
            return []
        except ObjectStoreError as e:
            logging.error(f"Failed to retrieve successful members from private object storage for task {task_id}: {type(e).__name__} - {str(e)}")
            return []
        except Exception as e: # ObjectStoreError以外の予期せぬエラー
            logging.error(f"Unexpected error retrieving successful members from private object storage for task {task_id}: {type(e).__name__} - {str(e)}")
            return []
    ...
    def get_error_logs(task_id):
        try:
            ...
            logging.debug(f"error_logs.json not found in private object storage for task {task_id}. Returning empty list.")
            return []
        except ObjectStoreError as e:
            logging.error(f"Failed to retrieve error logs from private object storage for task {task_id}: {type(e).__name__} - {str(e)}")
            return []
        except Exception as e: # ObjectStoreError以外の予期せぬエラー
            logging.error(f"Unexpected error retrieving error logs from private object storage for task {task_id}: {type(e).__name__} - {str(e)}")
            return []
